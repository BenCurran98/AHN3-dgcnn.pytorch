...
def train(k, io, 
    ...
    Args:
        ...
        validation_prop (float, optional): Proportion of the dataset to use for validation. Defaults to 0.2.
    ...
    # make sure device actually has a GPU avaiilable
    ...
    train_loader = DataLoader(
        ...
        shuffle=True, drop_last=True, collate_fn = collate_pcs)

    validation_data = FugroDataset(split='validation', data_root=data_dir, 
                                validation_prop = validation_prop, 
                                classes = range(num_classes))
    validation_loader = DataLoader(
        validation_data, num_workers=8, batch_size=validation_batch_size,
        shuffle=True, drop_last=True, collate_fn = collate_pcs)

    device = torch.device("cuda" if cuda else "cpu")
    print("Using ", device)

    model = DGCNN(num_classes, num_features, k, dropout = dropout, 
    ...
    for epoch in range(start_epoch, epochs):
        ...
        with tqdm(train_loader, desc = "Training epoch {}".format(epoch)) as t:
            for data, labels in train_loader:
                mask = torch.tensor(train_data.create_train_mask(labels, data.shape[1], exclude_classes = exclude_classes))
                data, labels, mask = data.to(device), labels.to(device), mask.to(device)
                data = data.permute(0, 2, 1).float()
                batch_size = data.size()[0]
                opt.zero_grad()
                labels_pred, _ = model(data) # batch_size * num_points * num_classes
        ...
        with tqdm(validation_loader, desc = "Validation epoch {}".format(epoch)) as t:
        ...
        outstr = 'Validation %d, loss: %.6f, Validation acc: %.6f, Validation avg acc: %.6f' % (epoch,
                                                                                              validation_loss * 1.0 / count,
                                                                                              validation_acc,
                                                                                              avg_per_class_acc)
        io.cprint(outstr)
        writer_validation_accuracy.add_scalar('Validation/accuracy', validation_acc, epoch)
        writer_validation_balanced_accuracy.add_scalar('Validation/balanced_accuracy', avg_per_class_acc, epoch)
...
def train_args(args, io):
    ...
    train(
        ...
        validation_batch_size = args.test_batch_size,
