...
import prepare_data.data_prep_util as data_prep_util
import prepare_data.indoor3d_util as indoor3d_util

# Constants
data_dir = BASE_DIR
indoor3d_data_dir = os.path.join(data_dir, 'AHN3_as_S3DIS_RGB_NPY')
NUM_POINT = 4096
H5_BATCH_SIZE = 1000
...
# Set paths
filelist = os.path.join(ROOT_DIR, 'meta/all_data_label.txt')
data_label_files = [os.path.join(indoor3d_data_dir, line.rstrip()) for line in open(filelist)]
output_dir = os.path.join(data_dir, 'AHN3_as_S3DIS_RGB_HDF5')
...
for i, data_label_filename in tqdm(enumerate(data_label_files)):
    ...
    data, label = indoor3d_util.room2blocks_wrapper_normalized(data_label_filename, NUM_POINT, block_size=30.0, stride=15,
...
for i in range(h5_index):
    all_file.write(os.path.join('AHN3_as_S3DIS_RGB_HDF5', 'ply_data_all_') + str(i) + '.h5\n')
all_file.close()
